...
all computed survey-weighted from the merged PLFS data (weight = mult/no_qtr,
the multiplier that makes unit records stand for the workforce): α (chat-only
exposure), E2 share (ζ − α, the tooling-dependent slice), graduate share,
female share, urban share, young share (18–29), and log survey-weighted median
monthly earnings among earners. The earnings concept is salaried +
self-employed monthly earnings; casual daily wages are not yet folded in, so
the level is understated wherever casual work is common, and any group with no
positive-earnings records is given the cross-group median before clustering.
...
(The silhouette figures quoted here are from the build that preceded the
survey-weighting fix to the earnings feature; read the current ones off
`data/processed/occupation_typology_PRELIMINARY.meta.json` after the next run.)

**Output.** Five clusters with employment-weighted profiles (table in the
white paper §8b, regenerated by `make typology` rather than typed by hand):
frontier professionals; a tooling-exposed paperwork layer; managers & teachers;
the rural agrarian mass; urban manual & retail. Every profile row is computed
on the pooled unit records of the cluster, survey-weighted — in particular the
earnings row is a worker-level weighted median of the cluster, not an average
of its group-level medians, and the shares are worker shares. Cluster names are
resolved from the profile itself (most chat-exposed world → frontier, and so
on) because k-means labels are arbitrary integers that permute whenever the
features or the data change; no exhibit keys off a cluster id. The
